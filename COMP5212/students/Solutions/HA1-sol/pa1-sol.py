...
#
#Copyright Statement: The materials provided by the instructor in this 
# course are for the use of the students enrolled in the course. Copyrighted 
#course materials may not be further disseminated.
#
import numpy as np
import os
import math
...
def compute_softmax_loss(W, X, y, reg):
    ...
    num_train = X.shape[0]
    f = X.dot(W)
    f -= np.max(f) # For numerical staility
    firstTerm = -f[np.arange(num_train),y] # see formula for NLL
    secondTerm = np.log(np.sum(np.exp(f),axis=1))  # see formula for NLL
    loss = np.mean(firstTerm + secondTerm) # batch NLL/batch size
    
    loss += 0.5*reg*np.sum(W*W)  # add regularization
    
    # See partial derivatives of NLL
    dFirstTerm = np.zeros(f.shape)
    dFirstTerm[np.arange(num_train),y] = 1
    dSecondTerm = np.exp(f)/np.sum(np.exp(f),axis=1,keepdims=True)
    dW = np.dot(X.T, - dFirstTerm + dSecondTerm) # 
    dW /= num_train
