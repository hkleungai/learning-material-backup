...
const int CAPACITY = 1024;  //global constant for array size
typedef int ElementType;    // ElementType is "alias" to int during compilation
...
class List
{
 ...
 /******** Function Members ********/
   ...
   /***** insert and erase *****/
   void insert(const ElementType & item, int pos);
   /*----------------------------------------------------------------------
     ...
     Precondition:  item is the value to be inserted; there is room in
         ...
         0(head) <= pos <= mySize (as the last item). 
