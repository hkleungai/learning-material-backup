...
{
  int newBack = (myBack + 1) % QUEUE_CAPACITY;  //for wrap around
...
// simply decrement queue size
void Queue::dequeue()
{
  if ( !empty() )
    myFront = (myFront + 1) % QUEUE_CAPACITY;  // increase myFront, wrap around if necessary
  else
   {
     cerr << "*** Queue is empty -- "
