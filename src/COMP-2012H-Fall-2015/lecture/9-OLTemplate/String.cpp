...
// returning a const String makes it a rvalue
// support s1 += s2 += s3, making it s1 += (s2 += s3)
// can NOT have (s1 += s2) += s3, because it returns a const String
const String &String::operator+=( const String &right )
{
...
// constant function: return reference to character in String as rvalue
...
// s(4,0) returns the substring starting from the 5th character s[4]
String String::operator()( int index, int subLength ) const
{
  ...
  if ( index < 0 || index >= length || subLength < 0 )  
    return ""; // converted implicity to an empty String object 

   // determine length of substring
  int len;

  if ( ( subLength == 0 ) || ( index + subLength > length ) )
    len = length - index;  // returning the right part of the string
  else
    len = subLength;  // returning the middle part of the string
  ...
  String tempString( tempPtr );  // construct a String object
...
istream &operator>>( istream &input, String &s )
{
  ...
  input >> setw( 100 ) >> temp;  // get at most 100 characters
