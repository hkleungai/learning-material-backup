...
def compute_neural_net_loss(params, X, y, reg=0.0):
    ...
    z2 = np.dot(u1,W2)+b2 #logits
 
    z2 -= np.max(z2)  # for numerical stabilty
    norm = np.sum(np.exp(z2),axis=1)
    loss = np.mean(-z2[np.arange(N),y]+np.log(norm))
    ...
    #############################################################################
    #gradient of loss = np.sum(-scores[range(N),y]+np.log(rows))/N
    #with respect to z2
    dFirstTerm = np.zeros(z2.shape)
    dFirstTerm[np.arange(N),y] = 1
    dSecondTerm = np.exp(z2)/np.sum(np.exp(z2),axis=1,keepdims=True)
    dz2 = - dFirstTerm + dSecondTerm #
    dz2 /= N
    ...
    #gradient of z2 = np.dot(u1,W2)+b2 with respect to u1
    du1 = dz2.dot(W2.T)
    #gradient of relu
    dz1 = du1 * (z1 >0)

    #gradient of z1 = np.dot(X,W1)+b1 with respect to W1
    dW1 = X.T.dot(dz1)
...
def train(X, y, Xtest, ytest, learning_rate=1e-3, reg=1e-5, epochs=100, batch_size=20):
    ...
    # first layer is a relu layer, and second one is a softmax layer.
