...
// TODO Task 2: Implementing the Mining function.
...
// TODO Task 3: Implementing the ComputeFinalBalance function.
...
int main() {
    ...
    if (blockNumber ==1 || blockNumber == 2 || blockNumber ==3 || blockNumber == 4 || blockNumber ==5){
        ...
        blockNumber = 1;
        cout<< "Wrong input, The miner will only mine " << blockNumber << " blocks." << endl << endl;
        cout<< "--------------------------------------------------------------" << endl;
    }
    ...
    do{
        ...
        if (chooseMinerCommand == 1) {
            ...
            cout << "Wrong input, please input your choice again"<<endl;
        }
    } while (true);
    ...
    do{
        ...
        cout<< ".....Mining Start ...." << endl<< endl;
        if (chooseMinerCommand == 1) {
            cout<< "Alice is the second selected miner." << endl<<endl;
            AliceBalance = AliceBalance + Mining(1, blockNumber);
            break;
        } else if (chooseMinerCommand ==2) {
            cout<< "Bob is the second selected miner." << endl<<endl;
            BobBalance = BobBalance + Mining(2, blockNumber);
            break;
        } else if (chooseMinerCommand == 3){
            cout<< "Carl is the second selected miner." << endl<<endl;
            CarlBalance = CarlBalance + Mining(3, blockNumber);
            break;
        }
        else {
            cout << "Worry input, please input your choice again"<<endl;
        }
    } while (true);
    ...
    double deltaAliceBob =  ComputeTransferValuePBV(AliceBalance, BobBalance, finalBalance);
    AliceBalance = AliceBalance - deltaAliceBob;
    BobBalance = BobBalance + deltaAliceBob;

    double deltaAliceCarl =  ComputeTransferValuePBV(AliceBalance, CarlBalance, finalBalance);
    AliceBalance = AliceBalance - deltaAliceCarl;
    CarlBalance = CarlBalance + deltaAliceCarl;

    double deltaBobCarl =  ComputeTransferValuePBV(BobBalance, CarlBalance, finalBalance);
    BobBalance = BobBalance - deltaBobCarl;
    CarlBalance = CarlBalance + deltaBobCarl;
...
double Mining(int selectedMiner, int blockNumber) {
    ...
    switch (selectedMiner) {
        case 1:
            while (blockNumber >0){
                reward = reward + 10 * blockNumber;
                blockNumber --;
            }
            return reward;
        case 2:
            reward = reward + 20* blockNumber;
            return reward;
        default:
            do{
                blockNumber --;
                reward = reward + (40-5*blockNumber);
            } while (blockNumber >0);
            return reward;
    }
}
...
double ComputeTransferValuePBV(double fromBalance, double toBalance, double finalBalance) {
    if((fromBalance < finalBalance && toBalance < finalBalance) || (fromBalance >= finalBalance && toBalance >= finalBalance)) {
        return 0;
    }  else if (fabs(fromBalance-finalBalance) < fabs(toBalance-finalBalance)){
        return  fromBalance < toBalance? -fabs(fromBalance-finalBalance): fabs(fromBalance-finalBalance);
    } else {
        return fromBalance < toBalance? -fabs(toBalance-finalBalance): fabs(toBalance-finalBalance);
    }
...
double RandomInitBalance(double dMin, double dMax) {
    ...
    return double(dMin + f * (dMax - dMin));
}
...
// This function is used to check whether two double-type variables are equal.
bool isDoubleEqual (double a, double b) {
